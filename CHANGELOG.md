...
#### 日付ナビゲーション追加
- 今日やること画面に「←前へ / 次へ→」ボタン追加
- URLパラメータ（`?date=yyyy-MM-dd`）で日付指定可能

#### 編集・修正ボタン改善
- スケジュール画面: 各曜日ヘッダー内に「編集」ボタン移動（Pencilアイコン付き）
- 週間レポート画面: 各日付に「修正」ボタン追加（今日やること画面へリンク）
- スケジュール編集画面: URLパラメータ（`?day=0-6`）で曜日指定可能

#### 月間レポート日付フォーマット
- 「yyyy年M月の月間レポート」→「yyyy/M の月間レポート」に変更

### ファイル変更一覧

| ファイル | 変更内容 |
|---------|---------|
| src/components/Sidebar.tsx | ナビラベル変更 |
| src/components/BottomNav.tsx | ナビラベル変更 |
| src/app/(authenticated)/dashboard/page.tsx | 日付ナビ追加、表記変更 |
| src/app/(authenticated)/schedule/page.tsx | 編集ボタン移動、表記変更 |
| src/app/(authenticated)/schedule/edit/page.tsx | URLパラメータ対応、表記変更 |
| src/app/(authenticated)/weekly/page.tsx | 修正ボタン追加 |
| src/app/(authenticated)/monthly/page.tsx | 日付フォーマット変更 |
