  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-lg border border-[#E5E5E5] p-6 mb-6 text-center">
        <div className="text-3xl font-bold text-[#DC4C3E]">
          {Math.floor(totalMinutes / 60)}時間{totalMinutes % 60}分
        </div>
        <div className="text-sm text-[#666666] mt-2">今月の合計達成時間</div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-[#E5E5E5] p-4">
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {['月', '火', '水', '木', '金', '土', '日'].map((day) => (
            <div
              key={day}
              className="text-center text-xs text-[#666666] py-1"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 gap-1">
          {calendarDays.map((date) => {
            ...
            return (
              ...
              >
                <span
                  className={isToday ? 'text-[#DC4C3E] font-bold' : 'text-[#666666]'}
                >
                  {format(date, 'd')}
                </span>
                {minutes > 0 && (
                  <span className="text-[10px] text-[#666666]">{minutes}分</span>
                )}
              </div>
            )
          })}
        ...
        {/* 凡例 */}
        <div className="flex justify-center items-center gap-2 mt-4 pt-4 border-t border-[#E5E5E5]">
          <span className="text-xs text-[#666666]">少ない</span>
          ...
          <span className="text-xs text-[#666666]">多い</span>
        </div>
      </div>
    </div>
  )
