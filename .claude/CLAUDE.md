...
## セマンティックバージョニング（ver X.Y.Z）

| 部分 | 名前 | 上げるタイミング |
|------|------|------------------|
| **X** | メジャー | 大きな変更、互換性がなくなる変更 |
| **Y** | マイナー | 新機能追加（既存機能は壊さない） |
| **Z** | パッチ | バグ修正、小さな改善 |

## バージョン表記の場所
- `src/components/Sidebar.tsx` - サイドバー下部
- `src/app/login/page.tsx` - ログイン画面下部

## デプロイ時のルール
- デプロイごとにバージョンを更新する
- 新機能追加 → Y を上げる（例: 1.0.1 → 1.1.0）
- バグ修正・微調整 → Z を上げる（例: 1.1.0 → 1.1.1）
- 大規模リニューアル → X を上げる（例: 1.1.1 → 2.0.0）
